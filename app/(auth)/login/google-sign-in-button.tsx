...
// "use client" — initiates the OAuth flow in the browser via Supabase.
// Kept separate from the page so the page remains a Server Component.
...
export default function GoogleSignInButton({ next }: Props) {
  ...
  async function handleGoogleSignIn() {
    ...
    try {
      ...
      if (oauthError) {
        console.error('[GoogleSignInButton] OAuth error:', oauthError.message)
        setError("Couldn't sign in — please try again.")
        setIsLoading(false)
      }

      // On success: Supabase redirects the browser to Google.
      // This component unmounts — no further state updates needed.
    } catch (err) {
      console.error('[GoogleSignInButton] Unexpected error:', err)
