...
// 8 active categories only
const VALID_CATEGORIES = [
  'restaurant','bar','film','tv','music','book','city','activity'
] as const
...
export async function POST(request: Request) {
  try {
    ...
    catch { return NextResponse.json<ApiResponse<null>>({ data: null, error: 'Invalid request body' }, { status: 400 }) }

    if (!body.title?.trim())
      return NextResponse.json<ApiResponse<null>>({ data: null, error: 'Title is required' }, { status: 400 })
    if (!body.category || !VALID_CATEGORIES.includes(body.category as typeof VALID_CATEGORIES[number]))
      return NextResponse.json<ApiResponse<null>>({ data: null, error: 'Please select a category' }, { status: 400 })
    if (!body.source_name?.trim())
      return NextResponse.json<ApiResponse<null>>({ data: null, error: 'Source name is required' }, { status: 400 })

    const sourceType = body.source_type && VALID_SOURCES.includes(body.source_type as typeof VALID_SOURCES[number])
      ? body.source_type : 'friend'
