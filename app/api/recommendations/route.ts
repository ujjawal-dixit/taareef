...
export async function GET(request: NextRequest) {
  try {
    ...
    if (category && !isValidCategory(category)) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: 'Invalid category' }, { status: 400 }
      )
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.title.length > 500) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: 'Title is too long' }, { status: 400 }
      )
    }
    if (!body.category || !isValidCategory(body.category)) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: `Category must be one of: ${VALID_CATEGORIES.join(', ')}` }, { status: 400 }
      )
    }
    if (!body.source_type || !VALID_SOURCE_TYPES.includes(body.source_type)) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: 'Invalid source type' }, { status: 400 }
      )
    }
    if (!body.source_name || body.source_name.trim() === '') {
      return NextResponse.json<ApiResponse<null>>(
    ...
    if (body.notes && body.notes.length > 1000) {
      return NextResponse.json<ApiResponse<null>>(
        { data: null, error: 'Note is too long' }, { status: 400 }
      )
    }
    if (body.url) {
      try { new URL(body.url) } catch {
        return NextResponse.json<ApiResponse<null>>(
          { data: null, error: 'URL is not valid' }, { status: 400 }
        )
      }
    }
    ...
    return NextResponse.json<ApiResponse<Recommendation>>(
      { data: data as Recommendation, error: null }, { status: 201 }
    )
  } catch (err) {
