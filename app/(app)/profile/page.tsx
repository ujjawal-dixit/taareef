...
// SHIPPED version.
// Back to vault: NOT a small text link. A proper button with weight and presence.
// Sign out: clearly separated, at the very bottom, never clips browser chrome.
// Parallel queries. Loading skeleton via loading.tsx.
...
export default async function ProfilePage() {
  ...
  sourcesResult.data?.forEach(r => { counts[r.source_name] = (counts[r.source_name] ?? 0) + 1 })
  ...
  return (
    <div style={{
      ...
      // Safe padding — enough for browser chrome + nav on any mobile browser
      paddingBottom: '140px',
    }}>

      {/* ── BACK TO VAULT ─────────────────────────────────────────────────
          This is not a footnote. It is the primary navigation.
          The user arrived here from the nav bar — the way home is
          a visible, weighted button at the top.
          Design: full-width pill, neon border, neon text, neon glow.
          Not small. Not dim. It says "this is how you go home."
      ──────────────────────────────────────────────────────────────── */}
      ...
      <div style={{ padding:'0 20px 20px', display:'flex', gap:'10px' }}>
        {[
          ...
          <div key={s.label} style={{
            ...
            <div style={{
              fontFamily:'var(--f-display)', fontWeight:400, fontStyle:'italic',
              fontSize:'34px', lineHeight:1, color:'#1fce94',
              textShadow:'0 0 18px rgba(31,206,148,0.45)', marginBottom:'4px',
            }}>{s.n}</div>
            <div style={{
      ...
      {/* Divider */}
      <div style={{ height:'0.5px', margin:'0 20px 18px', background:'rgba(240,230,200,0.05)' }} />

      {/* Feedback */}
      <div style={{ padding:'0 20px' }}>
        <ProfileClient
          userEmail={email} userName={name}
          saveCount={savedCount} topSource={topSource}
        />
      </div>
      ...
      {/* Sign out — at the very bottom, clearly separated */}
      <div style={{ padding:'0 20px' }}>
        <SignOutButton />
      </div>
