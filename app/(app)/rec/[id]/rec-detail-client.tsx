...
// Build full open-view meta — more detail than stacked.
// This is the "deciding" layer — everything the user needs to act.
function buildMeta(category: Category, meta: Record<string, unknown>): string {
  ...
  switch (category) {
    ...
    case 'listen': {
      ...
      const epLength = typeof meta.avg_episode_length === 'string' ? meta.avg_episode_length : null
      const category_label = typeof meta.category_label === 'string' ? meta.category_label : null
      const known_for = typeof meta.known_for === 'string' ? meta.known_for : null
      const active_since = meta.active_since ? `Since ${meta.active_since}` : null

      if (subtype === 'podcast') {
        if (host)           p.push(host)
        if (category_label) p.push(category_label)
        if (epLength)       p.push(epLength)
      } else if (subtype === 'song') {
        if (artist)   p.push(artist)
        if (album)    p.push(album)
        if (duration) p.push(duration)
        if (year)     p.push(String(year))
      } else if (subtype === 'artist') {
        if (genre)        p.push(genre)
        if (active_since) p.push(active_since)
        if (known_for)    p.push(known_for)
      } else {
        // album
        if (artist) p.push(artist)
        if (genre)  p.push(genre)
    ...
    // ── DINE ──────────────────────────────────────────────────────
    // Open: Type · Neighbourhood · City (cuisine removed)
    case 'dine': {
    ...
    case 'do': {
      ...
      const distance   = typeof meta.distance === 'string' ? meta.distance : null
      const duration   = typeof meta.duration === 'string' ? meta.duration : null
      const involves   = typeof meta.involves === 'string' ? meta.involves : null
      if (location)   p.push(location)
      if (difficulty) p.push(difficulty)
      if (distance)   p.push(distance)
      if (duration)   p.push(duration)
      if (involves)   p.push(involves)
      break
    }

    // ── VISIT ─────────────────────────────────────────────────────
    // Open: Venue · City · Director/Company (for Play) · Run dates
    // Dates rendered with urgency color separately — NOT in this string
    case 'visit': {
      const venue    = typeof meta.venue === 'string' ? meta.venue : null
      const city     = typeof meta.city === 'string' ? meta.city : null
      const director = typeof meta.director === 'string' ? meta.director
...
// Urgency levels for Visit dates — same logic as card component
    ...
    <div style={{ minHeight: '100vh', background: '#111111' }}>
      ...
      <div style={{ padding: '20px 20px 100px' }}>
        ...
        <div style={{ marginBottom: '20px' }}>
          ...
          {/* Visit — urgency-conditional date
              4 states: info (dim) / soon (cerulean 78%) / urgent (cerulean 100% + dot) / closed (strikethrough) */}
          {rec.category === 'visit' && (() => {
            ...
            const styleMap: Record<string, React.CSSProperties> = {
              info:   { color: 'rgba(30,159,235,0.55)', marginTop: '5px' },
              soon:   { color: 'rgba(30,159,235,0.80)', marginTop: '5px' },
              urgent: { color: 'rgba(30,159,235,1.0)', fontWeight: 500, marginTop: '5px', display: 'flex', alignItems: 'center', gap: '5px' },
              closed: { color: 'rgba(255,255,255,0.22)', textDecoration: 'line-through', marginTop: '5px' },
            }
            return (
              <div style={{ fontFamily: 'var(--f-body)', fontSize: '11px', fontWeight: 300, ...styleMap[urgency] }}>
          ...
          {/* Dine — what to order, shown if present */}
          {rec.category === 'dine' && typeof meta.what_to_order === 'string' && meta.what_to_order.trim() !== '' && (
            <div style={{ marginTop: '8px', display: 'flex', alignItems: 'baseline', gap: '6px' }}>
              <span style={{ fontFamily: 'var(--f-ui)', fontSize: '8px', fontWeight: 700, letterSpacing: '1.5px', textTransform: 'uppercase', color: `rgba(${cfg.vividRgb},0.55)`, flexShrink: 0 }}>
                Order
              </span>
              <span style={{ fontFamily: 'var(--f-body)', fontSize: '11px', fontWeight: 400, color: 'rgba(255,255,255,0.72)' }}>
