...
// Full card detail. Category verb. Reaction. Tell source.
// Notes editable inline. All saved to Supabase via PATCH.
...
export function RecDetailClient({ recommendation: init, categoryConfig: cfg }: Props) {
  ...
  async function markExperienced() {
    const updated = await update(rec.id, { status: 'experienced' })
    if (updated) { setRec(updated); toast(`Marked as ${cfg.verb} ✦`, 'success') }
  }

  async function setReaction(reaction: Reaction) {
    const updated = await update(rec.id, { reaction })
    if (updated) {
      setRec(updated)
  ...
  async function saveNotes() {
    ...
    const updated = await update(rec.id, { notes: val })
    if (updated) { setRec(updated); setNotesDirty(false) }
  }

  function shareWithSource() {
    const verb = cfg.verb
    const msg  = rec.reaction === 'loved'
      ? `Finally ${verb} ${rec.title} — you were so right. Thank you ❤️`
      : `${verb.charAt(0).toUpperCase() + verb.slice(1)} ${rec.title} — it was great! Thanks for the rec`
    if (navigator.share) {
  ...
  return (
    <div style={{ maxWidth: '430px', margin: '0 auto', minHeight: '100dvh', background: '#080f0a', paddingBottom: '48px' }}>
      ...
      <div style={{
        ...
        {isExperienced && (
          <div style={{
            ...
            background: 'rgba(31,206,148,0.12)', border: '0.5px solid rgba(31,206,148,0.28)',
            borderRadius: '6px', padding: '3px 10px',
            fontFamily: 'var(--font-rajdhani), system-ui, sans-serif',
            fontSize: '9px', fontWeight: 700, letterSpacing: '0.08em', textTransform: 'uppercase',
            color: '#1fce94',
      ...
      <div style={{ padding: '20px 20px 0' }}>
        ...
        <h1 style={{
          ...
          color: 'rgba(240,230,200,0.95)', lineHeight: 1.1, marginBottom: '6px',
        }}>
          {rec.title}
        </h1>
        ...
        <div style={{ height: '0.5px', background: 'rgba(240,230,200,0.08)', marginBottom: '20px' }} />
                ...
                return (
                  <button
                    ...
                    style={{
                      ...
                      border: sel ? '1px solid rgba(31,206,148,0.40)' : '1px solid rgba(240,230,200,0.09)',
