...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const { data } = await supabase.from('recommendations').select('title,source_name').eq('id', id).single()
  if (!data) return { title: 'taareef' }
...
export default async function RecDetailPage({ params }: Props) {
  ...
  const { data: rec, error } = await supabase
    .from('recommendations').select('*').eq('id', id).eq('user_id', user.id).single()
